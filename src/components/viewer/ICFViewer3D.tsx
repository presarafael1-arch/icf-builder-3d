...
// =============================================
// External Engine: Camera controller (Fit/Center)
// =============================================
...
function exPx(p: unknown): number | undefined {
  if (p == null) return undefined;
  if (Array.isArray(p)) return typeof p[0] === 'number' ? p[0] : undefined;
  if (typeof p === 'object') {
    const obj = p as Record<string, unknown>;
    if (typeof obj.x === 'number') return obj.x;
    if (typeof obj['0'] === 'number') return obj['0'] as number;
  }
  return undefined;
}

function exPy(p: unknown): number | undefined {
  if (p == null) return undefined;
  if (Array.isArray(p)) return typeof p[1] === 'number' ? p[1] : undefined;
  if (typeof p === 'object') {
    const obj = p as Record<string, unknown>;
    if (typeof obj.y === 'number') return obj.y;
    if (typeof obj['1'] === 'number') return obj['1'] as number;
  }
  return undefined;
}

function computeExternalBBox(layout: NormalizedExternalAnalysis): ExternalBBox | null {
  ...
  // Prefer wall offsets (most robust)
  for (const w of walls) {
    const left = (w.offsets?.left as unknown[]) || [];
    const right = (w.offsets?.right as unknown[]) || [];
    ...
    // Direct wall start/end (fallback)
  ...
  // Fallback: nodes (some engines provide better snapped nodes)
  if (!isFinite(minX) || !isFinite(minY)) {
    for (const n of nodes) {
      // Support both {position:{x,y}} and legacy {pt:[x,y]} payloads carried through normalization
      const pos = (n as any).position ?? (n as any).pt;
      const x = exPx(pos);
      const y = exPy(pos);
      if (x == null || y == null) continue;
      minX = Math.min(minX, x);
  ...
  // Match ExternalEngineRenderer auto-centering: translate so bbox center becomes (0,0)
  const offsetX = -(minX + maxX) / 2;
  const offsetY = -(minY + maxY) / 2;

  const minXm = minX + offsetX;
  const maxXm = maxX + offsetX;
  const minZm = minY + offsetY;
  const maxZm = maxY + offsetY;

  // Height in meters (external engine works in meters)
  const wallHeightM = layout.wallHeight || (layout.courses.at(-1)?.z1 ?? 0.8);

  const min = new THREE.Vector3(minXm, 0, minZm);
  const max = new THREE.Vector3(maxXm, Math.max(0.2, wallHeightM), maxZm);
...
function ExternalCameraController({ normalized }: { normalized: NormalizedExternalAnalysis }) {
  const { camera, controls } = useThree();
  const prevKeyRef = useRef<string>('');

  const bbox = useMemo(() => computeExternalBBox(normalized), [normalized]);
  ...
  // Auto-fit when external geometry changes
  useEffect(() => {
    if (!bbox) {
      prevKeyRef.current = '';
      return;
    ...
    const key = [
      ...
      bbox.size.x.toFixed(3),
      bbox.size.z.toFixed(3),
      bbox.size.y.toFixed(3),
    ].join('|');
    if (key !== prevKeyRef.current) {
      setTimeout(() => fit(), 0);
      prevKeyRef.current = key;
    }
  }, [bbox, normalized.walls.length, fit]);

  // Manual Fit View (same event used by internal mode)
  useEffect(() => {
    const onFit = () => fit();
    window.addEventListener('icf-fit-view', onFit as EventListener);
    return () => window.removeEventListener('icf-fit-view', onFit as EventListener);
      ...
      >
        ...
        {isExternalMode ? (
          <>
            ...
            {/* Auto Fit/Center for external engine geometry */}
            {hasExternalData && normalizedExternalAnalysis && (
              <ExternalCameraController normalized={normalizedExternalAnalysis} />
            )}
